...
async function bestDayToPostByEngagement(req,res,done){
    let engagement = null //Variable that contains the response for the facebook request
    /*The next seven variables are the ones that will have the prediction of each day of the week, that is the reason why
    they are identified by the name of the day */
    let monday = 0
    let tuesday = 0
    ...
    for(let i = 0; i <3; i++){
        ...
        if(socialytics == undefined || socialytics == ""){
            ...
            if(fail == undefined && igUser == undefined){
                ...
                let petition = {
                    method: "GET",
                    uri: `https://graph.facebook.com/v9.0/${ig_Id}/insights?metric=engagement&period=day&since=${sinceDate}&until=${untilDate}&access_token=${fbToken}`,
                    resolveWithFullResponse: true,
                    json: true
                }

                await request(petition).then((response) => {
                    engagement = response.body.data[0].values
                }).catch(function (err) {
                    fail = {
                        status: '400',
                        response: 'Bad Request',
        ...
        if(engagement != null){
            ...
            for(let i =0; i < engagement.length; i++){
                let day = moment(engagement[i].end_time).format('dddd')
                totalPrediction = totalPrediction+engagement[i].value 
                
                if(day == 'Monday'){
                    monday = monday + engagement[i].value
                }
                else if(day == 'Tuesday'){
                    tuesday = tuesday + engagement[i].value
                }
                else if(day == 'Wednesday'){
                    wednesday = wednesday + engagement[i].value
                }
                else if(day == 'Thursday'){
                    thursday = thursday + engagement[i].value
                }
                else if(day == 'Friday'){
                    friday = friday + engagement[i].value
                }
                else if(day == 'Saturday'){
                    saturday = saturday + engagement[i].value
                }
                else{ 
                    sunday = sunday + engagement[i].value
                }
            }
        }
        else{
    ...
    //For each variable of the week, it is calculated the confidence interval to get for each day the posible percent of probability of the best day to post 
    monday = (monday/totalPrediction)*100
    tuesday = (tuesday/totalPrediction)*100
    wednesday = (wednesday/totalPrediction)*100
    thursday = (thursday/totalPrediction)*100
