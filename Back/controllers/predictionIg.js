...
async function probableAmountOfReach(req,res,done){
    ...
    /*The next seven variables are the one that will have the prediction of each day of the week, that is the reason why
    they are identified by the name of the day */
    let monday = 0
    let tuesday = 0
    let wednesday = 0
...
async function bestDayToPostByEngagement(req,res,done){
    ...
    let totalEngagementP = 0 //Variable that contains the total addition of likes and comments of the request
    ...
    if((socialyticId == undefined || socialyticId == "")|| (media == undefined || media == ""))  {
        ...
        totalEngagementP = media.totalComments + media.totalLikes
        for(let i =0; i < media.mediaInfo.length; i++){
            /*To be able to calculate the best day to post, for each one of the values we get in the media, we need
            to check which day of the week it is and with that sum of the variable's value of that day, we need to calculate the total to be able to calculate de percent of probability*/
        ...
        //For each variable of the week, it is calculate the confidence interval to get for each day the posible percent of probability of the best day to post by engagements
        monday = (monday/totalEngagementP)*100
        tuesday = (tuesday/totalEngagementP)*100
        wednesday = (wednesday/totalEngagementP)*100
        thursday = (thursday/totalEngagementP)*100
        friday = (friday/totalEngagementP)*100
        saturday = (saturday/totalEngagementP)*100
        sunday = (sunday/totalEngagementP)*100
...
async function probableAmountOfImpressions(req,res,done){
    ...
    let probableimpressions = null //Variable that contains the response for the facebook request
    /*The next seven variables are the one that will have the prediction of each day of the week, that is the reason why
    they are identified by the name of the day */
    let monday = 0
    let tuesday = 0
    ...
    let saturday = 0
    let sunday = 0 
    let totalPredictionI = 0 //Variable that contains the total addition of the values of the request
    let fbToken = req.query.fbToken //Variable to handle the facebook token
    let socialytics = req.query.socialyticId //Variable to handle the user identification in the app
    let fail = null //Variable to handle some errors we need to put in some conditions
    let today = new Date() //Variable with the today actual date
    ...
    for(let i = 0; i <3; i++){
        ...
        if(socialytics == undefined || socialytics == ""){
            ...
            if(fail == undefined && igUser == undefined){
                ...
                let petition = {
                    method: "GET",
                    uri: `https://graph.facebook.com/v9.0/${ig_Id}/insights?metric=impressions&period=day&since=${sinceDate}&until=${untilDate}&access_token=${fbToken}`,
                    resolveWithFullResponse: true,
                    json: true
                }

                await request(petition).then((response) => {
                    probableimpressions = response.body.data[0].values
                }).catch(function (err) {
                    fail = {
                        status: '400',
                        response: 'Bad Request',
            ...
            //This else is ONLY use for the TDD, to test the Successfull case. The reason is that the facebook token comes from the view
            //and we can not make the request to facebook API to get the values
            probableimpressions = req.query.data
            i = 5
        }

        if(probableimpressions  != null){
            /*To be able to calculate the best day to post, for each one of the values we get in the request, we need
            to check which day of the week it is and with that, sum the value to the variable of that day
            
            And also, we need to calculate the total to be able to calculate de percent of probability*/
            for(let i =0; i < probableimpressions .length; i++){
                let day = moment(probableimpressions [i].end_time).format('dddd')
                totalPredictionI = totalPredictionI+probableimpressions[i].value 
                
                if(day == 'Monday'){
                    monday = monday + probableimpressions[i].value
                }
                else if(day == 'Tuesday'){
                    tuesday = tuesday + probableimpressions[i].value
                }
                else if(day == 'Wednesday'){
                    wednesday = wednesday + probableimpressions[i].value
                }
                else if(day == 'Thursday'){
                    thursday = thursday + probableimpressions[i].value
                }
                else if(day == 'Friday'){
                    friday = friday + probableimpressions[i].value
                }
                else if(day == 'Saturday'){
                    saturday = saturday + probableimpressions[i].value
                }
                else{ 
                    sunday = sunday + probableimpressions[i].value
                }
            }
        }
        else{
    ...
    //For each variable of the week, it is calculate the confidence interval to get for each day the posible percent of probability of impressions
    monday = (monday/totalPredictionI)*100
    tuesday = (tuesday/totalPredictionI)*100
    wednesday = (wednesday/totalPredictionI)*100
    thursday = (thursday/totalPredictionI)*100
    friday = (friday/totalPredictionI)*100
    saturday = (saturday/totalPredictionI)*100
    sunday = (sunday/totalPredictionI)*100
...
module.exports = {
    ...
    bestDayToPostByEngagement,
    probableAmountOfImpressions
}
